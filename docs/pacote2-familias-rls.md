...
| 4b | 10 | Botão "Contestar" (só membro) em Transações e Fatura, com categoria sugerida e comentário; tela **Contestações** (só admin) com Aceitar/Recusar (usa `resolver_contestacao`). Sem SQL novo. | Não | Reverter o PR |
| 5 | 9 | Bot por pessoa: só atende quem está em `telegram_vinculo` (conversa privada); `/vincular 123456` com código gerado na tela **Telegram** do app (admin e membro); tudo filtrado pela família da pessoa; triagem separada por chat (`open_finance_staging.chat_id`); lançamento grava `user_id` de quem lançou e `familia_id`; `/config` só admin. SQL `supabase/migrations/20260927120000_telegram_por_pessoa.sql` vincula o Telegram do dono antes de publicar o bot. O Secret `TELEGRAM_ALLOWED_CHAT_IDS` deixa de ser usado. | SQL pequeno + função publicada pelo dono | Publicar a versão anterior do bot |
